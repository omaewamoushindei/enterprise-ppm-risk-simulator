...
def query_software_utilization() -> float:
	sql = """
	SELECT AVG(utilization_pct) AS avg_software_utilization
	FROM employee_capacity_audit
	WHERE department = 'Software';
	"""
	df = run_query(sql)
	val = df.loc[0, "avg_software_utilization"]
...
def main() -> None:
	...
	software_utilization = query_software_utilization()
	...
	kpi_col2.metric("Software Utilization", f"{software_utilization:.2f}%")
